...
def import_h5(directory, experiment_title):
    ...
    bias = np.load(os.path.join(directory1, bias_file))
    slope = np.load(os.path.join(directory1, slope_file))
    ...
    # Convert exposure_times to a numpy array and remove duplicates
    exposure_times = np.unique(exposure_times)

    # Create a dictionary to store the denoised image arrays
    image_denoised_arrays = {}

    # Apply darkcount subtraction using the exposure time-dependent model
    for key in image_arrays.keys():
        denoised_array = []
        for img, t in zip(image_arrays[key], exposure_times):
            darkcount = bias + slope * t
            denoised = np.where(img > darkcount, img - darkcount, 0)
            denoised_array.append(denoised)
        image_denoised_arrays[key] = np.array(denoised_array)
    
    # Print the exposure times
