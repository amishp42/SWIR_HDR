...
def load_raw_data(directory, experiment_title, base_data_folder, extract=False):
    """
...
def adaptive_weight(z, Zmin, Zmax):
    """
...
def computeRadianceMap(images, exposure_times, Zmin_precomputed, Zmax_precomputed, smoothing_lambda=1000, return_all=False):
    logger.info("Starting computeRadianceMap function")
    
    intensity_samples, log_exposures, z_min, z_max = sampleIntensities(images, exposure_times, Zmin_precomputed, Zmax_precomputed)
    logger.info("Finished sampleIntensities function")
    
    response_curve, z_min, z_max = computeResponseCurve(intensity_samples, log_exposures, exposure_times, smoothing_lambda, adaptive_weight, Zmin_precomputed, Zmax_precomputed)
    ...
    for i in range(num_images):
        w = adaptive_weight(images[i], Zmin_precomputed[i], Zmax_precomputed[i])
...
def estimate_radiance(images, exposure_times, Zmin_precomputed, Zmax_precomputed):
    num_images, height, width = images.shape
    radiance = np.zeros((height, width))
    weight_sum = np.zeros((height, width))
    
    for i in range(num_images):
        weights = adaptive_weight(images[i], Zmin_precomputed[i], Zmax_precomputed[i])
        radiance += weights * images[i].astype(float) / exposure_times[i]
        weight_sum += weights
    
    return radiance / np.maximum(weight_sum, 1e-6)

def sampleIntensities(images, exposure_times, Zmin_precomputed, Zmax_precomputed, num_samples=50000):
    """Sample pixel intensities from the exposure stack."""
    num_images, height, width = images.shape
    z_min = np.min(Zmin_precomputed)  # Use the minimum value across all Zmin_precomputed
    ...
    radiance = estimate_radiance(images, exposure_times, Zmin_precomputed, Zmax_precomputed)
...
def computeResponseCurve(intensity_samples, log_exposures, exposure_times, smoothing_lambda, weighting_function, Zmin_precomputed, Zmax_precomputed):
    num_samples, num_images = intensity_samples.shape
    z_min = int(np.min(intensity_samples))  # Use actual minimum from data
    ...
    for z_k in range(z_min + 1, z_max):
        w_k = weighting_function(z_k, np.min(Zmin_precomputed), np.max(Zmax_precomputed))
    ...
    # # Center the response curve
    # z_mid = (z_min + z_max) // 2
    # response_curve -= response_curve[z_mid - z_min]

    return response_curve, z_min, z_max
...
def process_hdr_images(directory, experiment_title, base_data_folder, smoothing_lambda=1000, num_sets=None):
    ...
    # Load raw data
    data_dict = load_raw_data(directory, experiment_title, base_data_folder)
    ...
    for key, data in data_dict.items():
        ...
        Zmin_precomputed, Zmax_precomputed = precompute_zmax(Slinear, Sd, bias, exposure_times)
        print(f"Zmax_precomputed shape: {Zmax_precomputed.shape}")
        print(f"Zmin_precomputed shape: {Zmin_precomputed.shape}")
        
        # Calculate representative Zmin and Zmax values
        # Option 1: Use median values
        Zmin_sample = np.median(Zmin_precomputed)
        Zmax_sample = np.median(Zmax_precomputed)
        
        # Option 2: Use values from the center of the image
        # center_y, center_x = images.shape[1] // 2, images.shape[2] // 2
        # Zmin_sample = Zmin_precomputed[:, center_y, center_x].mean()
        # Zmax_sample = Zmax_precomputed[:, center_y, center_x].mean()
        
        radiance_map, response_curve, z_min, z_max, intensity_samples, log_exposures = computeRadianceMap(
            images, exposure_times, Zmin_precomputed, Zmax_precomputed, smoothing_lambda=smoothing_lambda, return_all=True
        )
...
def plot_weighting_function(weighting_function, Zmin, Zmax, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        pixel_values = np.linspace(Zmin, Zmax, 1000)
        weights = weighting_function(pixel_values, Zmin, Zmax)
...
def plot_response_curve(intensity_samples, log_exposures, response_curve, z_min, z_max, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        # Adjust log_exposures to center around 0 at z_mid
        z_mid = (z_min + z_max) // 2
        log_exposures_centered = log_exposures - response_curve[z_mid - z_min]
        
        for j in range(intensity_samples.shape[1]):
            ax.scatter(intensity_samples[:, j], log_exposures_centered[:, j], alpha=0.1, s=1, c='blue')
        
        pixel_values = np.arange(z_min, z_max + 1)
        ax.plot(pixel_values, response_curve - response_curve[z_mid - z_min], 'r-', linewidth=2, label='Fitted Response Curve')
    ...
    for plot_func, (row, col), title in plot_functions:
        try:
            ...
            if plot_func == plot_weighting_function:
                plot_func(adaptive_weight, data['Zmin_sample'], data['Zmax_sample'], ax=ax)
            elif plot_func == plot_log_radiance_map:
                plot_func(data['radiance_map'], ax=ax)
            else:
