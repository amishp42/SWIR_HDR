...
def load_denoised_data(directory, experiment_title, base_data_folder, extract=False):
    """
    ...
    for file in os.listdir(denoised_data_folder):
        if file.endswith("_denoised.npy"):
            key = file[:-13]  # Remove '_denoised.npy'
            file_path = os.path.join(denoised_data_folder, file)
...
def precompute_zmax(Slinear, Sd, bias, exposure_times):
    ...
    Zmin = np.round(Sd * exposure_times + bias).astype(int)
    Zmax = np.round(Slinear - Zmin).astype(int)
...
def adaptive_weight(z, Zmax):
    ...
    Zmin (numpy.ndarray): Minimum pixel intensity values (same shape as z)
...
def computeRadianceMap(images, exposure_times, Zmax_precomputed, smoothing_lambda=1000, return_all=False, initial_curve=None):
    logger.info("Starting computeRadianceMap function")
    
    intensity_samples, log_exposures, z_max = sampleIntensities(images, exposure_times, Zmax_precomputed)
    logger.info("Finished sampleIntensities function")
    
    response_curve, z_max = computeResponseCurve(intensity_samples, log_exposures, exposure_times, smoothing_lambda, adaptive_weight, Zmax_precomputed, initial_curve=initial_curve)
    ...
    for i in range(num_images):
        ...
        indices = np.clip(images[i].astype(int) - z_min, 0, len(response_curve) - 1)
    ...
    if return_all:
        return radiance_map, response_curve, z_max, intensity_samples, log_exposures
    else:
        return radiance_map
...
def sampleIntensities(images, exposure_times, Zmax_precomputed, num_samples=50000):
    ...
    return intensity_samples, log_exposures, z_max

def computeResponseCurve(intensity_samples, log_exposures, exposure_times, smoothing_lambda, weighting_function, Zmax_precomputed, initial_curve=None):
    num_samples, num_images = intensity_samples.shape
    z_min = 0
    z_max = int(np.max(intensity_samples))  # Use actual maximum from data
    ...
    # Handle initial curve if provided
    if initial_curve is not None:
        if len(initial_curve) != intensity_range:
            # Interpolate the initial curve to match the current intensity range
            x_old = np.linspace(0, 1, len(initial_curve))
            x_new = np.linspace(0, 1, intensity_range)
            f = interp1d(x_old, initial_curve, kind='cubic', fill_value='extrapolate')
            x0 = f(x_new)
        else:
            x0 = initial_curve

        # Use the initial guess to set bounds for lsq_linear
        lower_bounds = x0 - 0.1 * np.abs(x0)  # Allow 10% deviation below
        upper_bounds = x0 + 0.1 * np.abs(x0)  # Allow 10% deviation above
        bounds = (lower_bounds, upper_bounds)

        # Use the initial guess in an iterative solver
        result = lsq_linear(mat_A, mat_b.ravel(), bounds=bounds, method='trf')
        response_curve = result.x

    else:
        # If no initial guess, use the standard least squares solver
        x = np.linalg.lstsq(mat_A, mat_b, rcond=None)[0]
        response_curve = x.flatten()

    return response_curve, z_max

def process_hdr_images(directory, experiment_title, base_data_folder, smoothing_lambda=1000, num_sets=None, initial_crf_path=None):
    final_data_folder = os.path.join(directory, base_data_folder, "radiance_map")
    os.makedirs(final_data_folder, exist_ok=True)

    # Load denoised data
    data_dict = load_denoised_data(directory, experiment_title, base_data_folder)
    ...
    # Load initial CRF if provided
    initial_curve = None
    if initial_crf_path and os.path.exists(initial_crf_path):
        initial_crf_data = np.load(initial_crf_path, allow_pickle=True)
        if 'response_curve_0' in initial_crf_data:
            initial_curve = initial_crf_data['response_curve_0']
    ...
    for i, (key, data) in enumerate(data_dict.items()):
        ...
        # Calculate representative Zmin and Zmax values
        Zmin_sample = 0
        Zmax_sample = np.median(Zmax_precomputed)
        
        radiance_map, response_curve, z_max, intensity_samples, log_exposures = computeRadianceMap(
            images, exposure_times, Zmax_precomputed, smoothing_lambda=smoothing_lambda, 
            return_all=True, initial_curve=initial_curve
        )
        
        # Use the first computed response curve as the initial curve for subsequent sets
        if i == 0 and initial_curve is None:
            initial_curve = response_curve
        
        radiance_map_filename = f"{key}_radiance_map.npy"
        radiance_map_path = os.path.join(final_data_folder, radiance_map_filename)
        np.save(radiance_map_path, radiance_map)
        logger.info(f"Radiance map saved to: {radiance_map_path}")
        ...
        processed_data.append({
            ...
            'Zmin_sample': Zmin_sample,
            'Zmax_sample': Zmax_sample
        })
...
def plot_weighting_function(weighting_function, z_max, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        pixel_values = np.linspace(0, z_max, 1000)
        weights = weighting_function(pixel_values, z_max)
        ax.plot(pixel_values, weights)
        ax.set_xlabel("Pixel Intensity")
        ax.set_ylabel("Weight")
        ax.set_title("Pixel-specific Weighting Function")
        ax.set_xlim(0, z_max)
        ax.grid(True)
...
def plot_crf_residuals(intensity_samples, log_exposures, response_curve, z_min, z_max, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        z_mid = (z_min + z_max) // 2
        for j in range(intensity_samples.shape[1]):
            predicted = response_curve[np.clip(intensity_samples[:, j].astype(int) - z_min, 0, len(response_curve) - 1)]
            residuals = (log_exposures[:, j] - predicted) - (log_exposures[:, j].mean() - predicted.mean())
...
def plot_response_function_literature(intensity_samples, log_exposures, response_curve, z_min, z_max, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        for j in range(intensity_samples.shape[1]):
            ax.scatter(log_exposures[:, j], intensity_samples[:, j], alpha=0.1, s=1, c='cyan')
        
        pixel_values = np.arange(z_min, z_max + 1)
        ax.plot(response_curve, pixel_values, 'k-', linewidth=2)
        
        ax.set_ylabel('Pixel Value Z')
        ax.set_xlabel('Log Exposure X')
        ax.set_title('Response Function (Literature Style)')
        ax.grid(True)
    except Exception as e:
        print(f"Error in plot_response_function_literature: {str(e)}")
        ax.text(0.5, 0.5, f"Error: {str(e)}", ha='center', va='center', transform=ax.transAxes)

def plot_log_radiance_map(radiance_map, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        log_radiance = np.log(radiance_map)
        vmin = np.percentile(log_radiance[np.isfinite(log_radiance)], 1)
        vmax = np.percentile(log_radiance[np.isfinite(log_radiance)], 99)
        im = ax.imshow(log_radiance, cmap='viridis', vmin=vmin, vmax=vmax)
        plt.colorbar(im, ax=ax, label='Log Radiance')
        ax.set_title('Log Radiance Map')
    except Exception as e:
        print(f"Error in plot_log_radiance_map: {str(e)}")
        ax.text(0.5, 0.5, f"Error: {str(e)}", ha='center', va='center', transform=ax.transAxes)
...
def capture_plots(data, adaptive_weight):
    ...
    plot_functions = [
        (plot_weighting_function, (0, 0), 'Pixel-specific Weighting Function'),
        (plot_response_curve, (0, 1), 'CRF Fitting'),
        (plot_log_log_crf, (1, 0), 'Log-Log CRF'),
        (plot_crf_residuals, (1, 1), 'CRF Residuals'),
        (plot_response_function_literature, (2, 0), 'Response Function (Literature Style)'),
        (plot_radiance_histogram, (2, 1), 'Radiance Histogram'),
        (plot_log_radiance_map, (3, 0), 'Log Radiance Map'),
        (plot_radiance_map, (3, 1), 'Linear Radiance Map')
    ]
    ...
    for plot_func, (row, col), title in plot_functions:
        try:
            ...
            if plot_func == plot_weighting_function:
                plot_func(adaptive_weight, data['z_max'], ax=ax)
            else:
...
def plot_log_log_crf(response_curve, z_min, z_max, ax=None):
    if ax is None:
        ax = plt.gca()
    try:
        pixel_values = np.arange(1, z_max + 1)  # Start from 1 to avoid log(0)
        full_response_curve = np.zeros_like(pixel_values, dtype=float)
        full_response_curve[z_min:z_max+1] = response_curve[1:]  # Skip the first value to match pixel_values
        
        ax.plot(pixel_values, full_response_curve, 'r-', linewidth=2, label='Fitted Response Curve')
        ...
        ax.legend()
        ax.grid(True)
        ax.set_xlim(1, z_max)  # Start from 1 due to log scale
        ax.axvline(x=z_min, color='r', linestyle='--', label='z_min')
        ax.legend()
    except Exception as e:
...
def generate_multi_page_report(processed_data, directory, experiment_title, adaptive_weight, base_data_folder):
    #experiment_folder = os.path.basename(os.path.normpath(directory))
    data_folder = os.path.join(directory, base_data_folder, "reports")
